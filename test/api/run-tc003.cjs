...
// Use the same local database configuration as AppModule, without printing secrets.
...
async function run() {
  ...
  try {
    ...
    try {
      // Only this run's uniquely named fixtures; no existing users/tables are touched.
